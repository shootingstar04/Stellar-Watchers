    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (islaunched)
        {
            ...
            else if (collision.CompareTag(Define.EnemyTag) && !collision.isTrigger)
            {
                ...
                Debug.Log(collision.name + " ¿¡°Ô " + 10 * (ProgressData.Instance.reinforcementCount + 1) + "ÀÇ µ¥¹ÌÁö¸¦ ÀÔÈû");
            }
        ...
        foreach (Collider2D collider in collider2Ds)
        {
            if (collider != null)
            {
                ...
                else if (collider.CompareTag(Define.EnemyTag))
                {
                    ...
                    Debug.Log(collider.name + " ¿¡°Ô " + 10 * (ProgressData.Instance.reinforcementCount + 1) + "ÀÇ µ¥¹ÌÁö¸¦ ÀÔÈû");
                }
